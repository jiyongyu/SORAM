...
void StreamTransformationFilter::LastPut(const byte *inString, size_t length)
{
   ...
   //printf("LastPut (in): ");
   //for (int i = 0; i < length; i++) printf("%c", inString[i]); printf("\n");
	...
	case ONE_AND_ZEROS_PADDING:
		...
		else
		{
         ...
         //printf("LastPut (out): ");
         //for (int i = 0; i < length; i++) printf("%c", space[i]); printf("\n");

         // C F: disabled padding for SW ascend (we don't want 65 bytes / cache line)
         if (0) {
			   if (m_padding == PKCS_PADDING)
			   {
				   byte pad = space[s-1];
